...
alert(
  "Hildebrando está en el aeropuerto el Dorado, ya le pagaron su salario ($2.500.000) y decidirá que comer"
);
  ...
  while (true) {
    ...
    switch (choice) {
      ...
      case "2":
        hSalary -= goodFood;
        console.log(
          "Hildebrando eligió el subway con gaseosa. Estará llenito y bien. Ahora tiene $" +
            hSalary +
            " disponibles."
        );
        return;
...
function adjustSuitcaseSize() {
  ...
  let minReductionFactor = Math.min(
    altoReductionFactor,
    largoReductionFactor,
    anchoReductionFactor
  );
  ...
  console.log(
    `Las nuevas dimensiones de la maleta de Hildebrando son: alto = ${nuevoAlto.toFixed(
      2
    )}cm, largo = ${nuevoLargo.toFixed(2)}cm, ancho = ${nuevoAncho.toFixed(
      2
    )}cm`
  );
}
...
const playGame = () => {
  ...
  alert(
    "Hildebrando ahora se juega 300.000 con el taxista en piedra papel o tijeras"
  );
  while (!userGame) {
  ...
  console.log(
    `Jugaste ${userGame} y el taxista jugó ${taxiGame}. El juego termina en ${gameResult}. Ahora tienes $${hSalary} disponibles.`
  );
};
  ...
  while (daysInMacondo < 4 && alive) {
    ...
    switch (userChoice) {
      case "1":
        ...
        const poolChoice = prompt(
          "El agua de la piscina huele raro. ¿Quieres meterte? \n1. Sí \n2. No"
        );
        if (poolChoice === "1") {
          // Si se mete, muere
          alive = false;
          console.log(
            "Hildebrando se metió a la piscina y se ahogó por demasiado cloro. Terminan las vacaciones."
          );
        }
        break;
      case "2":
        // Si elige ir de caminatas
        const walkChoice = prompt(
          "¿Quieres ir toda la caminata? \n1. Sí \n2. No"
        );
        if (walkChoice === "1") {
          // Si elige ir toda la caminata, llega a una hermosa cascada y se toma fotos
          console.log(
            "Hildebrando fue toda la caminata, llegó a una hermosa cascada y se tomó fotos."
          );
        } else {
          // Si no, se devuelve solo y se pierde
          alive = false;
          console.log(
            "Hildebrando se devolvió solo de la caminata y se perdió. Terminan las vacaciones."
          );
        }
        break;
      case "3":
      ...
      case "4":
        ...
        const hotelChoice = prompt(
          "Elige una actividad en el hotel: \n1. Bingo \n2. Bailar \n3. Casino y apuesta"
        );
        if (hotelChoice === "1") {
          // Si juega al bingo, se lo gana y aumenta su dinero
          hSalary += 200000; // Aumentamos su salario en 200.000
          console.log(
            "Hildebrando ganó el bingo y aumentó su dinero. Ahora tiene $" +
              hSalary +
              " disponibles."
          );
          ...
          console.log(
            "Hildebrando fue al casino y apostó. Ahora solo tiene $" +
              hSalary +
              " disponibles para el taxi de regreso."
          );
        } else {
